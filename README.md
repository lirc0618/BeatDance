...
配置阿里云百炼后，Feed 导入成功会在响应之后尝试为刚生成的参考片段创建分阶段教学
...
后台生成是尽力执行的增强能力，不是动作导入事务的一部分。如果服务在响应后立即
重启，尚未完成的任务不会自动恢复；动作分析仍可正常使用规则引导，可重新上传同一
参考素材触发生成。
...
独立调用 `POST /actions/{action_id}/reference` 时，只有同时提交该参考片段在完整 Feed
中的 `source_start_seconds` 才会生成并启用暂停教学计划。省略该参数只更新动作分析
参考，不会用错误的 0 秒时间轴覆盖现有 Feed 教学计划。
